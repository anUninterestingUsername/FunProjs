...
class Player:
    ...
    def get_no(self): #Returns player number
     ...
     #This function should be rewritten to be smarter!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    def play(self): #This function should be rewritten to be smarter
        ...
        if self.no == 1: #Seperate code from generic one to test effectiveness
            rank = 0
            bestC = None
                ...
                if self.check_playable(card):
                    ...
                    if not card.color:
                        ...
                        if rank == 0:
                            bestC = card
                        continue
                    for cardd in self.hand:
                        if card == cardd:
                            continue
                        if card.number and card.number == cardd.number:
                            cardRank += 1
                        if card.color == cardd.color:
                            cardRank += 2
                    if card.number == None or card.number == 0:
                        cardRank += 2
                    if cardRank>=rank:
                        rank = cardRank
                        bestC = card
...
while not won:
    ...
    if action == "Skip":
        ...
        #turn = increment(turn, len(players))
        #I think this is unfair since it makes draw 2/4 very op.
